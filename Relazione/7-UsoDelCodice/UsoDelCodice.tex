...
Il software è scritto in Python 3.13. Per eseguire l'applicazione e riprodurre i risultati sperimentali è necessario configurare l'ambiente virtuale installando le dipendenze dichiarate:
...
\begin{verbatim}
PYTHONPATH=. .venv/bin/python scripts/genera_immagine_panoramica.py
PYTHONPATH=. .venv/bin/python scripts/genera_immagine_tipologie_ostacoli.py
PYTHONPATH=. .venv/bin/python scripts/genera_immagine_cammini_liberi.py
PYTHONPATH=. .venv/bin/python scripts/genera_immagine_contesto.py
PYTHONPATH=. .venv/bin/python scripts/genera_immagine_esplorazione_potatura.py
\end{verbatim}
Ciascuno scrive il proprio PNG in \texttt{Relazione/images/}, sovrascrivendo il file esistente. Altri due script non producono immagini, ma riproducono verifiche numeriche citate in prosa nel Compito 4 e nelle Conclusioni: la verifica empirica del limite di ricorsione e il confronto supplementare fra potatura debole e forte su una griglia $100\times100$:
\begin{verbatim}
PYTHONPATH=. .venv/bin/python scripts/verifica_limite_ricorsione.py
PYTHONPATH=. .venv/bin/python scripts/verifica_potatura_100.py
\end{verbatim}

Tre ulteriori script riproducono gli esperimenti delle estensioni discusse nei Compiti 3 e 4: l'effetto del controllo dei componenti connessi su una coppia irraggiungibile (attenzione: la configurazione senza controllo esaurisce l'intero tempo limite di $600$ s), la verifica dell'oracolo A* con il calcolo del divario anytime (che genera anche la figura \texttt{divario\_anytime.png} e il file \texttt{divario\_anytime.json}) e il confronto di memoria e tempo fra la matrice di byte e la griglia a piani di bit:
\begin{verbatim}
PYTHONPATH=. .venv/bin/python scripts/verifica_componenti.py
PYTHONPATH=. .venv/bin/python scripts/verifica_oracolo_astar.py
PYTHONPATH=. .venv/bin/python scripts/verifica_griglia_bit.py
\end{verbatim}
